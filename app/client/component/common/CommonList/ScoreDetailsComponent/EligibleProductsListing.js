...
const EligibleProductsListing = ({
  ...
  const [showRestCreditCards, setShowRestCreditCards] = useState(false)
  const [showRestBanks, setShowRestBanks] = useState(false)
  const [showRestLoans, setShowRestLoans] = useState(false)
  ...
  return (
    <>
      ...
      {filteredDataCard?.length > 0 && (
        <>
          {filteredDataCard?.slice(0, 5)?.map((alldata, index) => {
            return (
              <>
                <div
          ...
          {showRestCreditCards &&
            filteredDataCard?.length > 5 &&
            filteredDataCard?.slice(5, filteredDataCard?.length)?.map((alldata, index) => {
              return (
                <>
                  <div
                    key={alldata?.product_id}
      ...
      {filteredBankAccountsData?.length > 0 && (
        <>
          {filteredBankAccountsData?.slice(0, 5)?.map((alldata, index) => {
            return (
              <>
                <div
                  key={index}
          ...
          {showRestBanks &&
            filteredBankAccountsData?.length > 5 &&
            filteredBankAccountsData?.slice(5, filteredBankAccountsData?.length)?.map((alldata, index) => {
              return (
                <>
                  <div
                    key={index}
            ...
            <div className='mt-[30px]'>
              {!showRestLoans ? (
                <PersonalLoanCards
                  ...
                  isTablet={isTablet}
                  size={size}
                  personalProductList={eligiblePersonalLoans?.slice(0, 5)}
                ...
                eligiblePersonalLoans?.length > 5 && (
                  <PersonalLoanCards
                    viewDetailsIndex={viewDetailsIndex}
                    isMobile={isMobile}
                    isTablet={isTablet}
                    size={size}
                    personalProductList={eligiblePersonalLoans?.slice(5, eligiblePersonalLoans?.length)}
                    setViewDetailsIndex={setViewDetailsIndex}
                    isDetailsPage={false}
                    hidePagination={true}
                    hideFilterNotFound={true}
...
export default EligibleProductsListing
